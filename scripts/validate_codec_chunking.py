...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-decode",
        action="store_true",
        help="Skip decode timing even if the codec exposes decode support.",
    )
    parser.add_argument(
        "--write-decoded",
...
def main() -> None:
    ...
    for chunk in chunks[: args.max_chunks]:
        if codec.supports_decode and not args.skip_decode:
    ...
    summary_fields = [
        ...
        f"median_encode_ms={statistics.median(encode_latencies):.2f}",
        f"tokens_per_chunk={token_lengths}",
    ]
    if decode_latencies:
        summary_fields.append(f"avg_decode_ms={sum(decode_latencies) / len(decode_latencies):.2f}")
        summary_fields.append(f"median_decode_ms={statistics.median(decode_latencies):.2f}")
    else:
        summary_fields.append("avg_decode_ms=skipped")
